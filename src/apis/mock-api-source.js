      ...
      function (_path, opts, e) {
        ...
        if (auth === JOSH_SMITH_BASIC_AUTH) {
          window.sessionStorage.setItem(
            SESSION_TOKEN_SESSION_STORAGE_KEY,
            generateId()
          )
          window.sessionStorage.setItem(
            LOGGED_IN_USER_SESSION_STORAGE_KEY,
            JOSH_SMITH_USERNAME
          )
        ...
        return generateErrorResponse(
          400,
          'You have provided incorrect credentials'
        )
      }
    )
    .get(
      ...
      function (_path, opts) {
        var sessionToken = window.sessionStorage.getItem(
          SESSION_TOKEN_SESSION_STORAGE_KEY
        )
        if (sessionToken === null) {
          return generateErrorResponse(
            401,
            'You must be logged in view your details'
          )
        }
        var auth = getRequestAuthOrErrorResponse(opts)
        if (typeof auth !== 'string') {
        ...
        var loggedInUser = window.sessionStorage.getItem(
          LOGGED_IN_USER_SESSION_STORAGE_KEY
        )
        return new Response(
          JSON.stringify(
    ...
    .get(
      ...
      function (_path, opts) {
        ...
        if (sessionToken === null) {
          return generateErrorResponse(
            401,
            'You must be logged in to view your patients'
          )
        }
        var auth = getRequestAuthOrErrorResponse(opts)
        if (typeof auth !== 'string') {
    ...
    .get(
      ...
      function (path, opts) {
        ...
        if (sessionToken === null) {
          return generateErrorResponse(
            401,
            'You must be logged in to view patient details'
          )
        }
        var auth = getRequestAuthOrErrorResponse(opts)
        if (typeof auth !== 'string') {
        ...
        var patientId = new URL(path, window.location.origin).pathname
          .split('/')
          .filter(Boolean)
          .pop()
        if (
